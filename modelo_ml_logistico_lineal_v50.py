...
Objetivos:
- Regresión logística: probabilidad de incumplir SLA interno de 5 DH.
...
OPTIONAL_DEFAULTS = {
    ...
    "DIAS_RESTANTES_DH": np.nan, "FECHA_ACTUALIZACION": pd.NaT,
...
def prepare(source: pd.DataFrame) -> Tuple[pd.DataFrame, int]:
    ...
    for c in [
        ...
        "DIAS_EN_ESTADO_DH", "DIAS_RESTANTES_DH"
    ]:
        df[c] = _num(df[c])
    ...
    df["Y_ATRASO_5DH"] = np.where(df["DH_TOTAL"].notna(), (df["DH_TOTAL"] > SLA_DH).astype(float), np.nan)
...
def add_prior_history(df: pd.DataFrame) -> pd.DataFrame:
    ...
    base_rate = float(complete["Y_ATRASO_5DH"].mean()) if len(complete) else 0.15
...
def run_pipeline(source: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(full) < 100 or full["Y_ATRASO_5DH"].nunique() < 2:
    ...
    y_train_cls = df.loc[train_idx, "Y_ATRASO_5DH"].astype(int).to_numpy()
    y_test_cls = df.loc[test_idx, "Y_ATRASO_5DH"].astype(int).to_numpy()
    ...
    y_final_cls = df.loc[full_idx, "Y_ATRASO_5DH"].astype(int).to_numpy()
    ...
    df["SLA_CONSUMIDO_PCT"] = np.clip(current_days / SLA_DH, 0, 3)
    ...
    df["RIESGO_ATRASO"] = np.select(
        [df["ES_PENDIENTE"] & (df["DIAS_ACTUALES_DH"] > SLA_DH), prob >= RISK_HIGH, prob >= RISK_MEDIUM],
        ["ATRASADO", "ALTO", "MEDIO"], default="BAJO"
    )
    df["PREDICCION_SLA"] = np.where((prob >= 0.50) | (pred_days > SLA_DH), "PROBABLE INCUMPLIMIENTO", "PROBABLE CUMPLIMIENTO")
    df["BANDA_DIAS_PREDICHOS"] = pd.cut(
        df["DH_PREDICHO_ML"], [-0.001, 5, 7, 10, np.inf],
        labels=["0-5 DH", "6-7 DH", "8-10 DH", ">10 DH"], include_lowest=True
    ).astype(str)
    ...
    df["ESTADO_SLA"] = np.select(
        [df["ES_PENDIENTE"] & (df["DIAS_ACTUALES_DH"] > SLA_DH), df["ES_PENDIENTE"], df["DH_TOTAL"] > SLA_DH],
    ...
    df["TASA_ATRASO_ENTRENAMIENTO"] = float(full["Y_ATRASO_5DH"].mean())
    ...
    output_cols = [
        ...
        "MAE_DIAS_TEST", "RMSE_DIAS_TEST", "R2_DIAS_TEST", "TASA_ATRASO_ENTRENAMIENTO",
